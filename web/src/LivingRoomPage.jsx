  ...
  return (
    ...
    >
      ...
      <div className="lrCardBody">
        ...
        {(file.tags || []).length > 0 && (
          <div className="lrCardTags">
            {(file.tags || []).slice(0, 3).map((tag) => (
              <span key={tag} className="lrCardTag">{tag}</span>
            ))}
          </div>
        )}
      </div>
...
// ────────────────────────────────────────────────────────────
// 子组件：LrTagFilterBar（搜索框 + 横向 tag 筛选栏）
// ────────────────────────────────────────────────────────────
function LrTagFilterBar({ availableTags = [], tagFilter = [], onTagFilterChange, extraClass = "" }) {
  const [search, setSearch] = useState("");
  const visibleTags = search.trim()
    ? availableTags.filter((t) => t.toLowerCase().includes(search.trim().toLowerCase()))
    : availableTags;
  return (
    <div className={`lrTagFilterBar${extraClass ? ` ${extraClass}` : ""}`}>
      <div className="lrTagSearchBox">
        <input
          type="search"
          className="lrTagSearchInput"
          placeholder="搜索标签…"
          value={search}
      ...
      <button
        type="button"
        className={`lrTagPill${!tagFilter.length ? " active" : ""}`}
      ...
      {visibleTags.map((tag) => (
        <button
          key={tag}
          type="button"
          className={`lrTagPill${tagFilter.includes(tag) ? " active" : ""}`}
          onClick={() => onTagFilterChange?.((prev) =>
...
// ────────────────────────────────────────────────────────────
function InlineGrid({ files, thumbMap: tmap, continueMap, focusedId, query, onQueryChange, onPlay, onFocus, onClose, searchRef, onColsChange, onRefreshThumb, availableTags, tagFilter, onTagFilterChange }) {
...
export default function LivingRoomPage() {
  ...
  const lrAvailableTags = useMemo(() => {
    const tagSet = new Set();
    for (const f of playableFiles) {
      for (const t of (f.tags || [])) { if (t) tagSet.add(t); }
    }
    return [...tagSet].sort((a, b) => a.localeCompare(b, "zh-CN"));
  }, [playableFiles]);

  // 所有可浏览文件（横向列表 / 网格共用），tag 过滤在此应用
  const allBrowseFiles = useMemo(() => {
    if (!lrTagFilter.length) return recent;
    return recent.filter((f) => lrTagFilter.some((t) => (f.tags || []).includes(t)));
  }, [recent, lrTagFilter]);
  ...
  // 批量预取缩略图（分批，thumbMap 更新时自动推进）
  useEffect(() => {
    if (!p2p || !recent.length) return;
    const batch = recent.filter((f) => {
      const k = thumbKey(f);
      return !thumbMap[k] && !thumbLoadingRef.current.has(k);
    }).slice(0, 20);
    for (const file of batch) {
      ensureThumb(file);
    }
  }, [p2p, recent, ensureThumb, thumbMap]);
