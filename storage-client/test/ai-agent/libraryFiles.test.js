...
test("file access explanation exposes policy boundaries and tool list", async () => {
  await withTempDir(async (root) => {
    ...
    assert.equal(result.currentStatus.policy.acceptsStorageRootAbsolutePath, true);
    assert.equal(result.currentStatus.policy.absolutePathInputScope, "storage-root-only");
    ...
    assert.equal(result.policy.allowRawTextRead, true);
    assert.equal(result.policy.allowBinaryRead, false);
    assert.equal(result.policy.acceptsStorageRootAbsolutePath, true);
    assert.equal(result.policy.absolutePathInputScope, "storage-root-only");
    assert.equal(result.policy.rawAbsolutePathExposed, false);
    assert.equal(result.policy.binaryReadAllowed, false);
    assert.equal(result.policy.writeRequiresConfirmation, true);
    assert.match(result.summary, /索引、fileId、相对路径/);
    assert.match(result.summary, /绝对路径会先归一化为相对路径/);
    ...
    assert.equal(result.canAccess.storageRootAbsolutePathInput, true);
...
test("file access policy matches the agent FileAccessPolicy contract", async () => {
  await withTempDir(async (root) => {
    ...
    assert.deepEqual(policy.accessBy, ["fileId", "relativePath", "storageRootAbsolutePath"]);
    ...
    assert.equal(policy.acceptsStorageRootAbsolutePath, true);
    assert.equal(policy.absolutePathInputScope, "storage-root-only");
    ...
    assert.equal(publicPolicy.acceptsStorageRootAbsolutePath, true);
    assert.equal(publicPolicy.absolutePathInputScope, "storage-root-only");
